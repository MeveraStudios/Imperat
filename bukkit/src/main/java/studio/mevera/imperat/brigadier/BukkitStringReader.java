...
public class BukkitStringReader extends StringReader {
    ...
    public BukkitStringReader(ExecutionContext<BukkitSource> ctx, CommandInputStream<BukkitSource> inputStream) {
        super(loadString(ctx, inputStream));
        setCursor(inputStream.currentRawPosition());
    }

    private static String loadString(ExecutionContext<BukkitSource> ctx, CommandInputStream<BukkitSource> inputStream) {
